  ...
  for (let d = maxDepth; d >= 1; d--) {
    ...
    if (s.left) {
      if (s.leftFlat) {
        ...
        ctx.closePath(); ctx.clip();
        maybeDrawGlyph(ctx, (near.l + far.l) / 2, CY, (far.b - far.t) * 0.30, shade, nx + lft.dx, ny + lft.dy, absFaceLeft);
        ctx.restore();
      }
    } else {
    ...
    if (s.right) {
      if (s.rightFlat) {
        ...
        ctx.closePath(); ctx.clip();
        maybeDrawGlyph(ctx, (near.r + far.r) / 2, CY, (far.b - far.t) * 0.30, shade, nx + rgt.dx, ny + rgt.dy, absFaceRight);
        ctx.restore();
      }
    } else {
      const wx_far  = Math.min(CANVAS_W, 3 * far.r  - 2 * CX);
