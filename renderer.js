    ...
    if (s.left) {
      if (s.leftFlat) {
        if (s.leftParallel && 3 * far.l - 2 * CX > 0) {
          ...
          ctx.clip();
          maybeDrawGlyph(ctx, far.l, (far.t + far.b) / 2, 120 / d, shade, fx + lft.dx, fy + lft.dy, absFaceLeft, palette.glyphColor);
          ctx.restore();
        }
      } else {
        fillPoly(ctx, [
    ...
    if (s.right) {
      if (s.rightFlat) {
        if (s.rightParallel && 3 * far.r - 2 * CX < CANVAS_W) {
          ...
          ctx.clip();
          maybeDrawGlyph(ctx, far.r, (far.t + far.b) / 2, 120 / d, shade, fx + rgt.dx, fy + rgt.dy, absFaceRight, palette.glyphColor);
          ctx.restore();
        }
      } else {
        fillPoly(ctx, [
