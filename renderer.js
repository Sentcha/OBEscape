    ...
    if (s.left) {
      if (s.leftFlat) {
        if (s.leftParallel && 3 * far.l - 2 * CX > 0) {
          ...
          ctx.clip();
          maybeDrawGlyph(ctx, CX - 280 / (d - 0.5), CY, 120 / (d - 0.5), shade, fx + lft.dx, fy + lft.dy, absFaceBack, palette.glyphColor);
          ctx.restore();
        }
      } else {
        fillPoly(ctx, [
    ...
    if (s.right) {
      if (s.rightFlat) {
        if (s.rightParallel && 3 * far.r - 2 * CX < CANVAS_W) {
          ...
          ctx.clip();
          maybeDrawGlyph(ctx, CX + 280 / (d - 0.5), CY, 120 / (d - 0.5), shade, fx + rgt.dx, fy + rgt.dy, absFaceBack, palette.glyphColor);
          ctx.restore();
        }
      } else {
        fillPoly(ctx, [
